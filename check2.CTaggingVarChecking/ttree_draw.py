...
def draw(inTREE:ROOT.TTree,
        ...
        binning:tuple=() ):
    if '_' in outTAG:
        raise NameError(f'[InvalidInput] draw() accept an invalid outTAG "{ outTAG }", "_" is preserved word!!!')
...
def MainFunc():
    ...
    overall_weight = 'mcweight *puwei * scalefactor_photon* jetP4Smear'
    drawing_hists = []
    drawing_hists.append( DrawMC_DeepFlavour  ( signfile.ttree, 'Call',
            'isMatched==1 && jetHadFlvr==4', overall_weight) )
    drawing_hists.append( DrawMC_DeepFlavour  ( signfile.ttree, 'C0SV',
            'isMatched==1 && jetHadFlvr==4 && jetSubVtxMass==0', overall_weight) )
    drawing_hists.append( DrawMC_DeepFlavour  ( signfile.ttree, 'Ball',
            'isMatched==1 && jetHadFlvr==5', overall_weight) )
    drawing_hists.append( DrawMC_DeepFlavour  ( signfile.ttree, 'B0SV',
            'isMatched==1 && jetHadFlvr==5 && jetSubVtxMass==0', overall_weight) )
    drawing_hists.append( DrawMC_DeepFlavour  ( signfile.ttree, 'Lall',
            'isMatched==1 && jetHadFlvr==0', overall_weight) )
    drawing_hists.append( DrawMC_DeepFlavour  ( signfile.ttree, 'L0SV',
            'isMatched==1 && jetHadFlvr==0 && jetSubVtxMass==0', overall_weight) )



    outfile = ROOT.TFile('sigMC.cjet_sideban.root', 'recreate')
    def writeTo(outFILE, histDICT):
        ...
        dirName = histDICT[first_key].GetName().split('_')[0]
        info(f'Creating directory "{ dirName }"')
