...
#sideFILE = '/afs/cern.ch/user/l/ltsai/eos_storage/condor_summary/2022EE_GJet/stage2/stage2_GJetDataDataSideband.root'
...
signFILE = '/home/ltsai/ReceivedFile/GJet/latestsample/2022EE_MiniAODv12/stage2/stage2_GJetPythiaFlat.root'
...
def mainfunc(ptRANGEs:array):
    ...
    #pts = [ 200,220,250,300,500,800,1000,1500 ]
    #binpt = lambda hNAME: ROOT.ROOT.RDF.TH1DModel( hNAME, 'pt', len(pts)-1, array('f',pts) )
    def binpt(hNAME):
        print(f'[GenerateHist] {hNAME}')
        return ROOT.ROOT.RDF.TH1DModel( hNAME, 'pt', len(ptRANGEs)-1, ptRANGEs)
...
if __name__ == "__main__":
    print(f'[Inputfile] {signFILE}')
    if Path(signFILE).is_file() == False:
        raise IOError(f'[Invalid input file] MC "{signFILE}" does not existed in system')
    print(f'[Outputfile] {outputFILE}')

    import sys
    pt_ranges = array('f', [float(v) for v in sys.argv[1:]])
    print(f'[Use Pt Binning]')
    for idx  in range(len(pt_ranges)-1):
        pPtBin = idx+1 # pt bin from 1 to N
        ptL = pt_ranges[idx]
        ptR = pt_ranges[idx+1]
        print(f'   {pPtBin:02d}: {ptL:4.0f} - {ptR:4.0f}')
    mainfunc(pt_ranges)
